...
// Initialize Eyedropped if supported
const initEyeDropper = () =>{
    if ("EyeDropper" in window){
        ...
        pickBtn.addEventListener("click", async () =>{
            try{
                ...
                XPathResult.style.display = "grid";
    ...
    const hexToRgb = (hex) => {
        const r = parceInt(hex.slice(1, 3), 16);
    const g = parceInt(hex.slice(3, 5), 16);
    const b = parceInt(hex.slice(5, 7), 16);
    return 'rgb(${r}, ${g}, ${b}';
    };
